...
importProblemRun0 pool = do
    ...
    cCentrality    <- loadCriterion df "Centrality"
    cConnectivity  <- loadCriterion df "Connectivity"
    cAccessibility <- loadCriterion df "Accessibility"
    cVisibility    <- loadCriterion df "Visibility"
...
importProblemRun1 pool = do
    ...
    flip runSqlPool pool $ do
      -- Id of the firt problem (Sep-Nov 2016)
      let pId = toSqlKey 1
      repsert pId (ScenarioProblem sctaskpreview sctaskfile "Empower Shack scenario 2" 0.001)
      cIdCentrality    <- upsertCriterion cCentrality
      cIdConnectivity  <- upsertCriterion cConnectivity
      cIdAccessibility <- upsertCriterion cAccessibility
      cIdVisibility    <- upsertCriterion cVisibility
...
loadCriterion :: FilePath -> String -> IO Criterion
loadCriterion fp name = do
    cHtml <- Text.readFile $ fp </> name <.> "html"
    cImg  <- BS.readFile $ fp </> name <.> "png"
    cIcon <- fmap decodeUtf8 . BS.readFile $ fp </> name <.> "svg"
    return $ Criterion (pack name) cHtml cImg cIcon
