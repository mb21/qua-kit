...
def generate_tex(js):
    ...
    for service in js:
        for i in range(len(js[service]["inputs"])):
            js[service]["inputs"][i]["json"] = json.dumps(js[service]["inputs"][i]["type"], indent=2, sort_keys=True)
        for i in range(len(js[service]["outputs"])):
            js[service]["outputs"][i]["json"] = json.dumps(js[service]["outputs"][i]["type"], indent=2, sort_keys=True)
